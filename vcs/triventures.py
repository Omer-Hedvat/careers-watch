"""
Triventures portfolio adapter.

URL:       https://www.triventures.vc/portfolio/
Structure: Static HTML + client-side filters. All company cards (Healthcare,
           Fintech, Cyber) are present in the raw HTML. External website links
           are directly on each card.
"""
import re
...
_INDEX_URL = "https://www.triventures.vc/portfolio/"
_OWN_DOMAIN = "triventures.vc"
...
def fetch_portfolio() -> list[dict]:
    """Scrape Triventures portfolio and return list of {company_name, company_website}."""
    try:
        resp = httpx.get(_INDEX_URL, headers=HEADERS, follow_redirects=True, timeout=30)
        resp.raise_for_status()
        html = resp.text
    except Exception as e:
        print(f"Triventures: error fetching portfolio - {e}")
        return []
    ...
    print(f"Triventures: found {len(results)} portfolio companies")
    return results
